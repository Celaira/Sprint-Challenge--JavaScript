...
1. Describe the biggest difference between `.forEach` & `.map`.
    ...
    `.map()` creates a new array whereas `.forEach()` only loops through one.
...
4. Describe the four rules of the 'this' keyword.
    ...
    3. The `new` keyword.
    4. Whenever JS' `.call()` or `.apply()` methods are used.

5. Why do we need super() in an extended class?

    To tell the base class that it now has a child to give its attributes to.
